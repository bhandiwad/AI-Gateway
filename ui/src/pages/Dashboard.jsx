          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              ...
              <StatCard
                icon={DollarSign}
                label="Total Cost"
                value={`$${stats?.total_cost?.toFixed(4) || '0.00'}`}
                color="bg-green-500"
              />
              <StatCard
                icon={Clock}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Top Models</h3>
                  ...
                  {stats?.top_models?.slice(0, 5).map((model, index) => (
                    <div key={model.model} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-800">{model.model}</p>
                          <p className="text-xs text-gray-400">{model.provider}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-medium text-gray-800">{formatNumber(model.requests)}</p>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">{user?.is_admin ? 'Admin' : 'Standard'}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500">Rate Limit</span>
                    <span className="font-medium">{user?.rate_limit} req/min</span>
                  </div>
                  <div>
